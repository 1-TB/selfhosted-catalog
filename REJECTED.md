...
| pyLoad | No official image resolves under any namespace tried (`pyload/pyload`, `pyload/pyload-ng`) - both return zero tags from the registry API, and the project's docs domain (docs.pyload.net) doesn't resolve either. The commonly-referenced Docker Hub image appears to no longer exist under that name. |
| Cronmaster | `ghcr.io/fccview/cronmaster` resolves fine (pinnable tags, amd64+arm64), but its own docker-compose requires `privileged: true`, `pid: host`, and a bind-mounted `/var/run/docker.sock` just to edit the host's real crontab - well past what a normal volume/env-var model can express safely, and not something to run un-sandboxed behind a public subdomain. |
| Kestra | `kestra/kestra` resolves fine (pinnable tags, amd64+arm64), but every deployment mode (`server local`, `server standalone`) requires passing that as an explicit container command rather than an env var or default entrypoint - this schema has no command-override field, same issue as SiYuan. OSS-edition OIDC/SSO support is also unclear from the docs. |
| Ryot | `ignisda/ryot` resolves fine (pinnable tags, amd64+arm64) and only needs Postgres alongside it, but the docs disagree with themselves on the listening port (8000 in one place, 5000 via `SERVER_BACKEND_PORT` in another) and the Dockerfile has no `EXPOSE` to settle it - port is handled by a shell script/Caddyfile not visible in the repo. Not worth guessing at the one field this catalogue exists to get right. |
