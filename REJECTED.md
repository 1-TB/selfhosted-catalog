...
| LinkAce | `linkace/linkace` resolves fine (pinnable tags, e.g. `v2.6.1`), but its docs site (linkace.org) is client-side rendered and returned no usable content to fetch, and the only docker-compose file in the `Kovah/LinkAce` GitHub repo is the dev/build-from-source one (separate MariaDB/Postgres, PHP, nginx and Redis containers for a local build) rather than the documented production single-image setup. Couldn't confirm the production image's port, env vars or `needs` with confidence; left out rather than guess. |
