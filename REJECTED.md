...
| Rustypaste | `orhunp/rustypaste` resolves fine (pinnable tags, amd64+arm64), but the binary is built `FROM scratch` and hard-fails on startup if `config.toml` isn't present at `/app/config.toml` - there's no fallback to defaults or an env-only mode. The schema has no way to seed a config file's contents on first run, only directories and env vars, so a plain install would crash-loop until someone manually drops a config.toml in. |
